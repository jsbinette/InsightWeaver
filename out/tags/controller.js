...
var InstructionsController = class {
  ...
  transformTags(tags) {
    let transformedData = {};
    let sortedEntries = Object.entries(tags).sort((a, b) => a[0].localeCompare(b[0]));
    for (const [filePath, tags2] of sortedEntries) {
      let fileData = [];
      for (const [tag, entries] of Object.entries(tags2)) {
  ...
  async getInstructions() {
    return (await this.processTags(this.transformTags(this.tags))).join("\n");
  }
  /// Jan2025 JSB 
  ...
  async saveToWorkspace() {
    ...
    if (getExtensionConfig().view.files.inFiles) {
      ...
      if (workspaceFolder) {
        ...
        let output = await this.processTags(this.transformTags(this.tags));
  ...
  loadFromWorkspace() {
    if (!this._isWorkspaceAvailable())
      return;
    if (getExtensionConfig().view.files.inFiles) {
      const filePath = path2.join(vscode2.workspace.workspaceFolders[0].uri.fsPath, ".instructions", "tags.json");
    ...
    function deserializeTag(serializedTag) {
      const start = new vscode2.Position(serializedTag.range[0].line, serializedTag.range[0].character);
      const end = new vscode2.Position(serializedTag.range[1].line, serializedTag.range[1].character);
